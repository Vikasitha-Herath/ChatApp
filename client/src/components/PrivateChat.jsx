...
export default function PrivateChat({ room, onRoomUpdate }) {
  ...
  useEffect(() => {
    ...
    socket.on('newPrivateMessage', (msg) => {
      if (msg.room === room.roomId) { setMessages(prev => [...prev, msg]); setTimeout(scrollToBottom, 50); }
    });
    socket.on('messageLimitReached', ({ roomId }) => { if (roomId === room.roomId) setShowPayment(true); });
    socket.on('paymentRequired',     ({ roomId }) => { if (roomId === room.roomId) setShowPayment(true); });
    socket.on('chatUnlocked',        ({ roomId }) => { if (roomId === room.roomId) setRoomData(prev => ({ ...prev, isPaid: true })); });
    socket.on('roomUpdated', ({ roomId, lastMessage }) => {
      if (roomId === room.roomId) { setRoomData(prev => ({ ...prev, lastMessage })); onRoomUpdate?.(); }
    });
    ...
    return () => {
      socket.off('newPrivateMessage'); socket.off('messageLimitReached');
      socket.off('paymentRequired');  socket.off('chatUnlocked');
      socket.off('roomUpdated');      socket.off('userTyping');
    };
  ...
  const handleTyping = () => {
    ...
    typingTimeoutRef.current = setTimeout(() => socket.emit('typing', { room: room.roomId, isTyping: false }), 1500);
  };

  const sendMessage = (e) => {
    e.preventDefault();
    if (!input.trim() || !socket) return;
    if (!roomData?.isPaid && (roomData?.messageCount || 0) >= FREE_LIMIT) { setShowPayment(true); return; }
  ...
  const handlePaymentSuccess = async () => {
    try {
      await axios.post('/api/payment/confirm', { roomId: room.roomId, paymentIntentId: 'demo_' + Date.now() });
  ...
  const isConsecutive = (msgs, i) =>
    i > 0 && msgs[i - 1].sender?._id === msgs[i].sender?._id &&
  ...
  if (roomData?.status === 'pending') {
    return (
      <div className="flex flex-col h-full items-center justify-center text-center p-8">
        ...
        <h3 className="text-lg font-bold text-white mb-2" style={{ fontFamily: 'Syne,sans-serif' }}>Sync Request Pending</h3>
        <p className="text-slate-400 text-sm">Waiting for <strong className="text-white">{otherUser?.username}</strong> to accept</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-6 py-4 glass flex items-center gap-3"
        ...
        {isPaid ? (
          ...
          <div className="flex items-center gap-1.5 bg-violet-500/10 border border-violet-500/20 rounded-full px-3 py-1">
            <svg className="w-3.5 h-3.5 text-violet-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
            <span className="text-xs text-violet-400 font-semibold">{remainingFree} free left</span>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-slate-500 text-sm mt-1">Start syncing! First {FREE_LIMIT} messages are free.</p>
          </div>
        ) : (
          messages.map((msg, i) => {
      ...
      {/* Locked banner */}
      {isLocked && (
        <div className="px-4 py-3 bg-violet-500/10 border-t border-violet-500/20 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <svg className="w-4 h-4 text-violet-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
            <span className="text-sm text-violet-400 font-medium">3 free messages used. Unlock to keep syncing!</span>
          </div>
          <button onClick={() => setShowPayment(true)}
            className="text-xs bg-violet-600 hover:bg-violet-500 text-white font-bold px-3 py-1.5 rounded-lg transition-all">
            Unlock $0.99
          </button>
        </div>
      )}
      ...
      {/* Input */}
      <div className="px-4 py-4 glass" style={{ borderTop: '1px solid rgba(124,58,237,0.12)' }}>
        <form onSubmit={sendMessage} className="flex items-center gap-3">
          <Avatar user={user} size="sm" />
          <div className="flex-1 relative">
            <input type="text"
              className={`input-field pr-12 py-2.5 text-sm ${isLocked ? 'opacity-50 cursor-not-allowed' : ''}`}
              placeholder={isLocked ? 'Unlock to keep syncing...' : `Message ${otherUser?.username}...`}
            ...
            <button type="submit" disabled={!input.trim() || isLocked}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-30 disabled:cursor-not-allowed transition-all active:scale-90">
