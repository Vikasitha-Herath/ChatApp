...
export default function RightSidebar({ onStartPrivateChat, existingRooms }) {
  ...
  const handleStartChat = async (targetUser) => {
    const existingRoom = existingRooms?.find(r =>
      r.participants.some(p => p._id === targetUser.userId)
    );
    if (existingRoom?.status === 'accepted') {
      onStartPrivateChat(existingRoom);
      return;
    }
    setRequesting(targetUser.userId);
    try {
      const { data } = await axios.post('/api/chat/private/request', { targetUserId: targetUser.userId });
      toast.success(`Chat request sent to ${targetUser.username}!`);
  ...
  return (
    <div className="flex flex-col h-full glass neon-border" style={{ borderLeft: '1px solid rgba(6,182,212,0.1)' }}>
      {/* Header */}
      <div className="p-4 border-b border-slate-800/60">
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-400" style={{ boxShadow: '0 0 6px rgba(52,211,153,0.8)' }} />
          <h2 className="font-bold text-white text-sm" style={{ fontFamily: 'Syne,sans-serif' }}>Online Members</h2>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {otherOnline.length === 0 ? (
          ...
          otherOnline.map(onlineUser => {
            ...
            return (
              <div key={onlineUser.userId} className="group flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-800/50 transition-all duration-200">
                ...
                <button
                  ...
                  className={`opacity-0 group-hover:opacity-100 flex-shrink-0 p-1.5 rounded-lg transition-all duration-200 ${
                    isAccepted ? 'bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 border border-cyan-500/20'
                    : isPending ? 'bg-amber-500/20 text-amber-400 border border-amber-500/20 cursor-default'
                    : 'bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white'
                  }`}
                  title={isAccepted ? 'Open chat' : isPending ? 'Request pending' : 'Start private chat'}>
                  {isLoading
      ...
      <div className="p-4 border-t border-slate-800/60">
        <div className="bg-slate-900/60 rounded-xl p-3 space-y-2">
          <p className="text-xs font-semibold text-slate-400" style={{ fontFamily: 'Syne,sans-serif' }}>Quick Stats</p>
          ...
          <div className="flex justify-between text-xs"><span className="text-slate-500">Free messages</span><span className="text-cyan-400 font-semibold">3 per chat</span></div>
