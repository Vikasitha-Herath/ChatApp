...
export default function GeneralChat() {
  ...
  useEffect(() => {
    ...
    return () => { socket.off('newGeneralMessage'); socket.off('userTyping'); };
  ...
  const handleTyping = () => {
    ...
    typingTimeoutRef.current = setTimeout(() => socket.emit('typing', { room: 'general', isTyping: false }), 1500);
  };
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-4 glass flex items-center gap-3"
        ...
        <div>
          <h2 className="font-bold text-white" style={{ fontFamily: 'Syne,sans-serif' }}>General</h2>
          <p className="text-xs text-slate-500">Everyone is synced here</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-slate-400 font-medium">Be the first to sync!</p>
          </div>
        ) : (
          messages.map((msg, i) => {
