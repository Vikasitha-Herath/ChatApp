    ...
    <div className="auth-container">
      <div className="auth-card animate-fade-in">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/20 mb-3 animate-pulse-glow">
            {step === STEPS.DONE
              ...
              : <svg className="w-7 h-7 text-cyan-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
            }
          </div>
        ...
        <div className="flex justify-center gap-2 mb-8">
          {[0, 1, 2, 3].map(i => (
            <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${stepIndex >= i ? 'bg-cyan-500 w-8' : 'bg-slate-700 w-4'}`} />
          ))}
        </div>
        ...
        {step === STEPS.EMAIL && (
          <form onSubmit={handleSendOTP} className="space-y-4">
            ...
            <button type="submit" className="btn-primary w-full flex items-center justify-center gap-2" disabled={loading}>
              {loading ? <><div className="w-4 h-4 border-2 border-slate-900 border-t-transparent rounded-full animate-spin" />Sending...</> : 'Send OTP'}
            </button>
          </form>
        )}
        ...
        {step === STEPS.OTP && (
          <form onSubmit={handleVerifyOTP} className="space-y-6">
              ...
              <div className="flex gap-2 justify-center">
                {otp.map((digit, i) => (
                  <input
                    ...
                    className="w-11 h-12 text-center text-xl font-bold bg-slate-900/80 border border-slate-700 focus:border-cyan-500 text-white rounded-xl outline-none transition-all duration-200"
                    value={digit}
            ...
            <button type="submit" className="btn-primary w-full flex items-center justify-center gap-2" disabled={loading}>
              {loading ? <><div className="w-4 h-4 border-2 border-slate-900 border-t-transparent rounded-full animate-spin" />Verifying...</> : 'Verify OTP'}
            </button>
            <button type="button" className="w-full text-sm text-slate-400 hover:text-cyan-400 transition-colors"
        ...
        {step === STEPS.RESET && (
          <form onSubmit={handleResetPassword} className="space-y-4">
            ...
            <button type="submit" className="btn-primary w-full flex items-center justify-center gap-2" disabled={loading}>
              {loading ? <><div className="w-4 h-4 border-2 border-slate-900 border-t-transparent rounded-full animate-spin" />Resetting...</> : 'Reset Password'}
            </button>
          </form>
        )}
